...
class TestPitchLevels:
    @pytest.mark.parametrize('mora_count', [2, 3, 4, 5])
    def test_reproduces_every_existing_pattern(self, mora_count):
    ...
    def test_single_mora(self):
        assert pitch_levels(1, set(), set()) == pitch_sequence(1, 0)
        assert pitch_levels(1, {1}, {1}) == pitch_sequence(1, 1)

    def test_multiple_rises_and_drops(self):
        assert pitch_levels(8, {2, 6}, {4}) == list('LHHHLHHH')

    def test_drop_on_final_mora_stays_high(self):
        assert pitch_levels(3, {2}, {3}) == ['L', 'H', 'H']

    def test_rise_and_drop_on_same_mora(self):
        assert pitch_levels(4, {2}, {2}) == ['L', 'H', 'L', 'L']
...
class TestRenderPhraseSvg:
    def test_returns_svg_element(self):
        svg = render_phrase_svg(5, {2}, {4}, {3})
        assert svg.startswith('<svg')
        assert svg.endswith('</svg>')

    def test_one_circle_per_mora(self):
        assert render_phrase_svg(5, {2}, {4}, set()).count('<circle') == 5

    def test_lines_connect_consecutive_dots(self):
        assert render_phrase_svg(5, {2}, {4}, set()).count('<line') == 4

    def test_hollow_circle_per_particle(self):
        svg = render_phrase_svg(8, {2, 6}, {4}, {3, 8})
        assert svg.count('fill="white"') == 2

    def test_no_hollow_circles_without_particles(self):
        assert 'fill="white"' not in render_phrase_svg(4, {2}, {3}, set())
    ...
    def test_high_and_low_dots_use_expected_heights(self):
        svg = render_phrase_svg(2, {2}, set(), set())
...
class TestSvgName:
    def test_all_sections(self):
        assert svg_name(5, {2}, {4}, {3}) == 'phrase_5_r2_d4_p3.svg'

    def test_multiple_positions_joined_with_dashes(self):
        assert svg_name(8, {2, 6}, {4}, {3, 8}) == 'phrase_8_r2-6_d4_p3-8.svg'

    def test_empty_sections_omitted(self):
        assert svg_name(3, {2}, set(), set()) == 'phrase_3_r2.svg'
        assert svg_name(3, set(), set(), {1}) == 'phrase_3_p1.svg'
        assert svg_name(3, set(), set(), set()) == 'phrase_3.svg'

    def test_positions_sorted(self):
        assert svg_name(9, {6, 2}, set(), set()) == 'phrase_9_r2-6.svg'
